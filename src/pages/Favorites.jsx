...
export default function FavoritesPage() {
  ...
  const loadFavorites = async () => {
    setIsLoading(true);
    try {
      const favoriteTracks = await Track.filter({ is_favorite: true }, '-created_date', 100);
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900">
      ...
      <div className="bg-white border-b border-slate-200 p-6 material-elevation-1">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 mb-1">Favorites</h1>
            <p className="text-slate-600">
              {tracks.length} favorite tracks
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        {isLoading ? (
            ...
            <Table>
              ...
              <TableBody className="dark:divide-slate-700">
                {filteredTracks.map((track, index) => (
                  <TrackContextMenu track={track} onUpdate={loadFavorites} key={track.id}>
                    <TableRow 
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-500">
            <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4"><Heart className="w-8 h-8" /></div>
            <h3 className="text-lg font-semibold mb-2">{searchQuery ? "No favorites found" : "No favorites yet"}</h3>
            <p className="text-sm text-center">{searchQuery ? "No favorites match your search." : "Drag tracks to the 'Favorites' tab in the sidebar to add them."}</p>
          </div>
        )}
      </div>
    </div>
