...
import TrackContextMenu from "../components/tracks/TrackContextMenu"; // Added import
...
export default function RecentlyPlayedPage() {
  ...
  const loadRecentlyPlayed = async () => {
    setIsLoading(true);
    try {
      // Get tracks with play history, sorted by last played
      const data = await Track.list('-last_played', 50);
      // Filter only tracks that have been played (have last_played date)
      const playedTracks = data.filter(track => track.last_played);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-slate-200 p-6 material-elevation-1">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 mb-1">Recently Played</h1>
            <p className="text-slate-600">
              {tracks.length} recently played tracks
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto p-6 bg-slate-50">
        {isLoading ? (
          <div className="bg-white rounded-xl material-elevation-1 p-4">
          ...
          <div className="bg-white rounded-xl material-elevation-1 overflow-hidden">
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow className="border-b border-slate-200">
                  ...
                  <TableHead className="font-semibold text-slate-700">Album</TableHead>
                  <TableHead className="font-semibold text-slate-700">Last Played</TableHead>
                  <TableHead className="font-semibold text-slate-700">Play Count</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredTracks.map((track, index) => (
                  <TrackContextMenu track={track} onUpdate={loadRecentlyPlayed} key={track.id}>
                    <TableRow 
                      className="group hover:bg-slate-50 transition-colors draggable-track cursor-pointer"
                    ...
                    >
                      ...
                      <TableCell className="py-3">
                        <div className="flex items-center gap-3">
                          <div 
                            className="w-10 h-10 bg-gradient-to-br from-slate-200 to-slate-300 rounded overflow-hidden flex-shrink-0"
                            style={{
                      ...
                      <TableCell className="py-3 text-slate-700">{track.artist}</TableCell>
                      <TableCell className="py-3 text-slate-600">{track.album}</TableCell>
                      <TableCell className="py-3 text-slate-600 text-sm">
                        {formatLastPlayed(track.last_played)}
                      </TableCell>
                      <TableCell className="py-3 text-slate-600 text-sm">
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-500">
            ...
            <p className="text-sm text-center">
              {searchQuery 
                ? "No recently played tracks match your search." 
